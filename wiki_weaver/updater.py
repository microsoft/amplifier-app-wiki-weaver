...
Two-layer refresh:
  ...
  Layer 2 — engine bundles managed by foundation's GitSourceHandler in
             ~/.amplifier/cache/bundles (currently just the context-intelligence
             CI hook -- see _LAYER2_SOURCES). Refreshed via foundation's public
             GitSourceHandler.update() (rmtree+reclone).
...
#
# NOTE (engine-020 compat fix): the "attractor-bundle" entry
# (amplifier-bundle-attractor) was removed from this list. wiki-weaver's own
# engine runs are always BARE (see engine_runner.py -- no run_pipeline() call
# passes bundle=), so nothing in the current codebase ever causes foundation
# to clone amplifier-bundle-attractor into ~/.amplifier/cache/bundles; the
# entry (and the ATTRACTOR_ROUTING_FLOOR_SHA compatibility-floor check that
# depended on it, below) tracked a repo that is no longer part of any live
# code path post-engine-repo-split. amplifier-bundle-attractor is now frozen
# and superseded by amplifier-bundle-dot-runner, which wiki-weaver consumes
# as a pyproject dependency (amplifier-module-pipeline-runner), not a cloned
# Layer-2 bundle -- so there is no equivalent "resolved commit" to floor-check
# here at all. context-intelligence remains: it IS still fetched at runtime
# (see engine_runner.CI_HOOK_SOURCE / _ci_overlay).
...
# ---------------------------------------------------------------------------
# Layer-2: attractor routing-contract compatibility floor -- RETIRED
# ---------------------------------------------------------------------------
#
# This section used to define check_attractor_routing_floor(), verifying that
# the locally-resolved "attractor-bundle" commit was at or past
# ATTRACTOR_ROUTING_FLOOR_SHA (attractor PR #89, transitively #88 -- a
# fail-safe-verdict-routing floor for pipeline/synthesize.dot's assess node).
#
# Retired (not repointed) post-engine-repo-split: the mechanism assumed the
# engine was a Layer-2 CLONED BUNDLE (amplifier-bundle-attractor, resolved via
# foundation's GitSourceHandler into ~/.amplifier/cache/bundles) whose commit
# could be floor-checked. That is no longer how the engine is consumed --
# amplifier-module-pipeline-runner is a pyproject dependency (Layer 1, an
# installed wheel; see _LAYER1_WHEEL_DEPS), and wiki-weaver's own engine runs
# are always BARE (engine_runner.py never passes bundle=), so there is no
# "resolved attractor-bundle commit" left to check at all --
# amplifier-bundle-attractor is frozen and superseded by
# amplifier-bundle-dot-runner. A version floor on amplifier-module-pipeline-
# runner itself (Layer 1, already tracked in _LAYER1_WHEEL_DEPS) would be the
# equivalent check now, if one is ever needed again.
