...
Sablefish and Tinderbox are also flagged as hosted-model alternatives that affect provider coverage [src: byo-agent-ecosystem-recon]. Ivo Petran surfaced a further adjacent-ecosystem signal in the July 7–9, 2026 team sync: Palisade Data's **Omniharness**, described in its own announcement as a meta-harness for combining, controlling, and sharing agents across multiple underlying frameworks — a different architectural bet than Halyard's single-bundle-per-host adapter model, since Omniharness's premise is orchestrating across harnesses rather than embedding one agent inside each host [6]. Ivo shared it alongside a companion Palisade blog post benchmarking coding agents against Palisade's own multi-million-line codebase [6] — a concrete, named instance of the long-horizon multi-million-line benchmarking work that Ingrid Vasquez separately flagged as a gap in the industry (see [[evaluations-workstream|Evaluations Workstream]]).

> TODO-VERIFY: The specific claims and methodology in Palisade Data's Omniharness announcement and its multi-million-line codebase benchmark post were shared as links in team chat, not independently reviewed or verified by the team as of the source date.
...
Naomi Aldridge raised a differentiator question in the July 9, 2026 team sync: she was not aware of any open-source agent harness — Pilotfish included — that ships with its own evals, and floated this as a potential Halyard differentiator [6]. Callum Whitfield's read was that each harness likely has internal evals but they are rarely shared externally [6]. A follow-up survey shared in the same chat (sourced from an assistant query Farrah Nkemdi relayed) found the opposite is closer to true for several of the harnesses already named in this recon's host-harness tier: Pilotfish ships `pilotfish-bench` under the Pilotfish/Driftline org; Openhearth's official benchmark repo describes itself as containing evaluation infrastructure for Openhearth agents; Rigsmith and mini-rigsmith are evaluated through the Rig-bench leaderboard maintained under the Rigsmith org; Corvid documents the harness and tools used for its own benchmarks in its benchmark folder; Slate shipped `slate-bench`, an announced open benchmark initiative; and Nimbus CLI's repo includes behavioral eval docs for validating agent behavior [6]. The survey's one-line finding: these are mostly official, project-maintained evals, while cross-agent rankings such as Rig-bench, the Longhaul suite, and Throughline Analysis are external comparison layers rather than each agent's own eval suite [6]. This refines rather than contradicts Callum's read: the harnesses do have evals, they are simply project-specific and not designed for cross-harness comparison — exactly the gap the team's cross-harness Halyard-agent benchmark goal and Ingrid Vasquez's long-horizon benchmark gap (below) both target; see [[evaluations-workstream|Evaluations Workstream]] for the fuller benchmark-gap context.

> TODO-VERIFY: The specific repo names and claims in this survey (e.g. `pilotfish-bench` under a "Pilotfish/Driftline org", the Openhearth benchmarks repo, Corvid's benchmark folder) were generated by a third-party assistant query relayed in chat, not independently confirmed by the team as of the source date.
...
[^6]: Hollis M - Team Sync chat (2026-07-07 – 2026-07-09) — Naomi Aldridge's harness-evals differentiator question, the assistant-sourced survey of which open-source agent harnesses ship their own evals, and Ivo Petran's Palisade Omniharness / multi-million-line codebase benchmark links
