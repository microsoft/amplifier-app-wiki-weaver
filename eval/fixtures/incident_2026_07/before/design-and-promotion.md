...
## Halyard Agent website and community storytelling (July 2026)

The July 7, 2026 planning session (Hollis, Naomi, Renée, Callum) activated Renée's role in the Halyard Agent release push with a specific new deliverable: **a simple marketing website** for Halyard Agent [2026-07-07 Agent-Planning]. The team's diagnosis was that the existing repo README, while comprehensive, is too technical and too comprehensive for the target audience — it lists all the different ways to use Halyard Agent and all the things it can do, but does not communicate a clear, simple call to action.

Renée's framing of the challenge: prior Halyard marketing efforts felt open-ended — *"okay, great, I want to get started. How do I do that?"* — and the team needed to hone in on who the target users are and what they want to do before designing the site [2026-07-07 Agent-Planning]. Her approach: rather than trying to communicate that Halyard is for everybody, focus on a **general user profile** that helps clarify which people to optimize the site for (see [[halyard-pilotfish-integration|Halyard Agent and Pilotfish Integration]] for the user profile the team converged on).

The site's primary promotion target is **Halyard Agent running under Pilotfish** — the most accessible entry point, requiring no deep understanding of bundles or harness internals [2026-07-07 Agent-Planning]. From there, the site can link to the Halyard Agent layer itself as something users can embed in other harnesses (Bramble, Quarry, custom builds). Hollis's framing: *"we can actually educate a little bit potentially on harness and go like, look, it's more than the model, it's what you bring"* [2026-07-07 Agent-Planning].

Renée also raised a key messaging question: whether the site should explicitly communicate what users are *missing out on* by using a general-purpose chat assistant as a harness directly — shifting from activity-based to outcome-based framing. The team agreed this was the right angle: those systems are still very activity-based, whereas Halyard Agent is designed to push past the activity loop and actually achieve outcomes [2026-07-07 Agent-Planning].

The team planned to test the "Halyard Agent" name and the site's messaging on the community chat group (the early-adopter circle that formed around the first public release) before committing — getting feedback from people who are not already inside the team's mental model [2026-07-07 Agent-Planning].
