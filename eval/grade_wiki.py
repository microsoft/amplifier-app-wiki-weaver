...
# GradeResult and grade_overview() (+ their constants/regexes) now live in the
# SHIPPED wiki_weaver package (wiki_weaver/grading.py), not here, because
# wiki_weaver/reweave.py (runtime code) needs to call grade_overview() as its
# free deterministic gate and eval/ is deliberately excluded from the
# installed wheel. Re-exported here so every existing caller in this module,
# this file's own CLI, and the eval/test_*.py suite keeps working unchanged —
...
# ---------------------------------------------------------------------------
#
# NOTE: these regexes/constants now live in wiki_weaver/grading.py and are
# imported (see top of file) rather than defined here. Left as a section
# marker for readers scanning this file's structure.
...
# NOTE: GradeResult now lives in wiki_weaver/grading.py and is imported (see
# top of file) rather than defined here.
...
# ---------------------------------------------------------------------------
#
# NOTE: grade_overview() now lives in wiki_weaver/grading.py and is imported
# (see top of file) rather than defined here — see the module docstring
# there for why (wiki_weaver/reweave.py, shipped runtime code, needs it as
# its free deterministic gate; eval/ is not shipped in the installed wheel).
